...
interface AnthropicUsageConnectedCardProps {
  onTest: () => Promise<void>
  onDisconnect: () => Promise<void>
  isLoading: boolean
}

export function AnthropicUsageConnectedCard({ onTest, onDisconnect, isLoading }: AnthropicUsageConnectedCardProps) {
    ...
    <Card className="border-2 border-green-200 bg-green-50/50 max-w-2xl mx-auto">
      <CardHeader>
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-600">Daily token consumption from Anthropic APIs</p>
          </div>
        </div>
      </CardHeader>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="bg-slate-50 border border-slate-200 rounded-lg p-3 text-xs text-slate-600">
          ...
          <div>We collect daily token consumption and request counts from your Anthropic organization. Usage data is shown on the dashboard after each analysis run.</div>
        </div>
