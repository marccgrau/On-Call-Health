#!/usr/bin/env python3
"""Standalone MCP server runner for On-Call Health.

Run this script to start the MCP server for use with Claude Desktop or other MCP clients.
The server communicates via stdio and requires an API key for authentication.
"""
import os
import sys
...
def main():
    ...
    # We need to build the MCP server without triggering the mcp_app creation
    # which fails because FastMCP doesn't expose an ASGI app in standalone mode
    ...
    # Import directly from auth.py to avoid triggering server.py import
    auth_spec = importlib.util.spec_from_file_location("auth", "app/mcp/auth.py")
    auth_module = importlib.util.module_from_spec(auth_spec)
    auth_spec.loader.exec_module(auth_module)
    require_user_api_key = auth_module.require_user_api_key

    # Import directly from serializers.py
    serializers_spec = importlib.util.spec_from_file_location("serializers", "app/mcp/serializers.py")
    serializers_module = importlib.util.module_from_spec(serializers_spec)
    serializers_spec.loader.exec_module(serializers_module)
    serialize_datetime = serializers_module.serialize_datetime
    serialize_github_integration = serializers_module.serialize_github_integration
    serialize_jira_integration = serializers_module.serialize_jira_integration
    serialize_linear_integration = serializers_module.serialize_linear_integration
    serialize_rootly_integration = serializers_module.serialize_rootly_integration
    serialize_slack_integration = serializers_module.serialize_slack_integration
    ...
    def _inject_api_key_from_env(ctx: Any) -> Any:
        """Inject API key from OCH_API_KEY environment variable into context for stdio-based MCP."""
        api_key = os.getenv("OCH_API_KEY")
        if not api_key:
            return ctx

        # Create a context object with headers attribute containing the API key
        class ContextWithHeaders:
            def __init__(self, original_ctx, api_key_value):
                self._original = original_ctx
                self.headers = {"X-API-Key": api_key_value}
                # Copy other attributes from original context
                for attr in dir(original_ctx):
                    if not attr.startswith('_') and attr != 'headers':
                        try:
                            setattr(self, attr, getattr(original_ctx, attr))
                        except:
                            pass

        return ContextWithHeaders(ctx, api_key)

    def _require_user_with_env_key(ctx: Any, db: Session):
        """Wrapper for require_user_api_key that injects API key from environment."""
        ctx_with_key = _inject_api_key_from_env(ctx)
        return require_user_api_key(ctx_with_key, db)
    ...
    async def analysis_start(
        ...
        db = _get_db()
        try:
            user = _require_user_with_env_key(ctx, db)
    ...
    def methodology_resource() -> str:
        ...
        return (
            "On-Call Health measures overwork risk using a two-dimensional model. "
            "It combines objective workload signals (incidents, "
    ...
    # Run server (FastMCP handles stdio communication)
    print("Starting On-Call Health MCP server...", file=sys.stderr)
    mcp_server.run()
