...
Takes an analysis ID and user email, and creates a copy of that analysis for the user.

Usage:
    python mock_data_helpers/assign_analysis_to_user.py --analysis-id 738 --user-email you@example.com --dry-run
    python mock_data_helpers/assign_analysis_to_user.py --analysis-id 738 --user-email you@example.com --apply
"""
import sys
import os
import argparse
...
# Always use local database for this script
os.environ["DATABASE_URL"] = "XXX"

# Add the backend directory to the Python path so 'app' is importable
...
def fetch_user(db, user_email: str) -> User:
...
def main():
    ...
    parser.add_argument('--user-email', type=str, required=True, help='Email of the user to assign the analysis to')
    ...
    try:
        ...
        target_user = fetch_user(db, args.user_email)
        ...
        print("[*] Processing...\n")
        assigned = assign_analysis_to_user(db, source_analysis, target_user, dry_run=args.dry_run)
        ...
        print(f"{'='*60}")
        if assigned:
            print(f"{'Would assign' if args.dry_run else 'Assigned'}: analysis {args.analysis_id} to {args.user_email}")
        else:
